...
if (Prototype.Browser.Opera) {
    Array.prototype.concat = function() {
        ...
        for (var i = 0, length = this.length; i <
                                              length; i++) array.push(this[i]);
        for (var i = 0, length = arguments.length; i < length; i++) {
            if (Object.isArray(arguments[i])) {
                for (var j = 0, arrayLength = arguments[i].length; j <
                                                                   arrayLength; j++)
...
if (Prototype.Browser.Opera) {
    ...
    $w('positionedOffset getOffsetParent viewportOffset').each(function(
            method) {
...
document.viewport = {
    getDimensions: function() {
        ...
        $w('width height').each(function(d) {
            var D = d.capitalize();
            dimensions[d] =
            (B.WebKit && !document.evaluate) ? self['inner' + D] :
        ...
        index: function(parentNode, reverse, ofType) {
            ...
            if (reverse) {
                for (var nodes = parentNode.childNodes, i = nodes.length -
                    ...
                    var node = nodes[i];
                    if (node.nodeType == 1 &&
                        (!ofType || node._counted)) node.nodeIndex = j++;
                }
            } else {
        ...
        child: function(nodes) {
            ...
            for (var i = 0, results = [], node; node = nodes[i]; i++) {
                for (var j = 0, child; child = node.childNodes[j]; j++)
                    if (child.nodeType == 1 &&
                        child.tagName != '!') results.push(child);
            }
            return results;
        },
...
Form.Methods = {
    ...
    getInputs: function(form, typeName, name) {
        ...
        for (var i = 0, matchingInputs = [], length = inputs.length; i <
                                                                     length; i++) {
            var input = inputs[i];
...
if (!document.getElementsByClassName) document.getElementsByClassName = function(
        instanceMethods) {
    function iter(name) {
