...
String.prototype.parseColor = function() {
    ...
    if (this.slice(0, 4) == 'rgb(') {
        ...
        if (this.slice(0, 1) == '#') {
            if (this.length == 4) for (var i = 1; i <
                                                  4; i++) color += (this.charAt(i) +
...
var Effect = {
    ...
    Transitions: {
        ...
        flicker: function(pos) {
            var pos = ((-Math.cos(pos * Math.PI) / 4) + 0.75) +
                      Math.random() / 4;
            return pos > 1 ? 1 : pos;
        },
        wobble: function(pos) {
                ...
                child.nodeValue.toArray().each(function(character) {
                    element.insertBefore(
                            new Element('span', {style: tagifyStyle}).update(
                                    character == ' ' ?
                                    String.fromCharCode(160) : character),
                            child);
                });
    ...
    toggle: function(element, effect) {
        ...
        Effect[element.visible() ?
               Effect.PAIRS[effect][1] :
               Effect.PAIRS[effect][0]](element, options);
    }
};
...
Effect.Base = Class.create({
    ...
    start: function(options) {
        ...
        if (options && options.transition ===
                       false) options.transition = Effect.Transitions.linear;
...
Effect.Highlight = Class.create(Effect.Base, {
    ...
    setup: function() {
        ...
        this._base = $R(0, 2).map(function(i) {
            return parseInt(this.options.startcolor.slice(i * 2 + 1, i * 2 +
                                                                     3), 16)
        }.bind(this));
        this._delta = $R(0, 2).map(function(i) {
            return parseInt(this.options.endcolor.slice(i * 2 + 1, i * 2 +
                                                                   3), 16) -
                   this._base[i]
        }.bind(this));
    },
    update: function(position) {
        this.element.setStyle({backgroundColor: $R(0, 2).inject('#', function(m,
                                                                              v,
                                                                              i) {
            return m + ((this._base[i] +
                         (this._delta[i] * position)).round().toColorPart());
        }.bind(this)) });
    },
    finish: function() {
...
Effect.Shake = function(element) {
    ...
    { x:  distance, y: 0, duration: split, afterFinishInternal: function(
        ...
        { x: -distance * 2, y: 0, duration: split *
                                            2,  afterFinishInternal: function(
                effect) {
            new Effect.Move(effect.element,
            { x:  distance * 2, y: 0, duration: split *
                                                2,  afterFinishInternal: function(
...
Effect.SlideUp = function(element) {
    ...
    return new Effect.Scale(element, window.opera ? 0 : 1,
            Object.extend({ scaleContent: false,
                ...
                afterUpdateInternal: function(effect) {
                    effect.element.down().setStyle({bottom:
                            (effect.dims[0] - effect.element.clientHeight) +
                            'px' });
                },
                afterFinishInternal: function(effect) {
...
Effect.Morph = Class.create(Effect.Base, {
    ...
    setup: function() {
        function parseColor(color) {
            if (!color ||
                ['rgba(0, 0, 0, 0)','transparent'].include(color)) color = '#ffffff';
        ...
        this.transforms = this.style.map(function(pair) {
            ...
            return {
                ...
                originalValue: unit == 'color' ? parseColor(originalValue) :
        ...
        while (i--)
            ...
            transform.unit == 'color' ? '#' +
                                        (Math.round(transform.originalValue[0] +
                                                    (transform.targetValue[0] -
                                                     transform.originalValue[0]) *
                                                    position)).toColorPart() +
                                        (Math.round(transform.originalValue[1] +
                                                    (transform.targetValue[1] -
                                                     transform.originalValue[1]) *
                                                    position)).toColorPart() +
                                        (Math.round(transform.originalValue[2] +
                                                    (transform.targetValue[2] -
                                                     transform.originalValue[2]) *
                                                    position)).toColorPart() :
...
if (document.defaultView && document.defaultView.getComputedStyle) {
    ...
    Element.getStyles = function(element) {
        ...
        styles = Element.CSS_PROPERTIES.inject({ }, function(results,
                                                             property) {
