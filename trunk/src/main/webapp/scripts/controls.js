...
Autocompleter.Base = Class.create({
    ...
    updateElement: function(selectedElement) {
        ...
        if (this.options.select) {
            var nodes = $(selectedElement).select('.' + this.options.select) ||
                        [];
            if (nodes.length >
                0) value = Element.collectTextNodes(nodes[0], this.options.select);
        } else
    ...
    getTokenBounds: function() {
        ...
        var tp;
        for (var index = 0, l = this.options.tokens.length; index <
                                                            l; ++index) {
...
Ajax.InPlaceEditor.prototype.initialize.dealWithDeprecatedOptions = function(
    ...
    fallback('cancelControl', (options.cancelLink ? 'link' :
                               (options.cancelButton ? 'button' :
                                options.cancelLink == options.cancelButton ==
                                false ? false : undefined)));
    fallback('okControl', (options.okLink ? 'link' :
                           (options.okButton ? 'button' :
